...
#Figure 18 Setting 4: logistic 
load("fig18_set4_adapt.RData")
out_exp1_adapt= out_exp1
load("fig18_set4.RData")
...
names(plotdata1)<-"fdr"
plotdata1$type<-as.factor(rep(c('AdaPTGMM','DECONV','HAMT',
...
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('AdaPTGMM','DECONV','HAMT',
                                'ASH','ASH 1','NPMLE B'),each=length(u)))
plotdata2$n<- rep(u,6)
