...
exp_1<-function(n,mu0,u,q,reps){
  ...
  for(nn in 1:length(u)){
    ...
    result<-foreach(r = 1:reps,.packages=c("ashr","Rmosek"))%dopar%{
      ...
      ash_out = ash(x,sig,mixcompdist = "+uniform",alpha=0,outputlevel=3,
                    mode="estimate")
      ...
      ####################################### alpha = 1 ##################
      ash_out = ash(x,sig,mixcompdist = "+uniform",alpha=1,outputlevel=3,
                    mode="estimate")
...
save.image('fig7_set4_ash.RData')
