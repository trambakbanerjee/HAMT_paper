...
save.image('plot_figs_22_24.RData')
...
i1 = which(sig_n[sig_sort,nn]>1)
i2 = which(sig_n[sig_sort,nn]<=1)
pointxy.1 <- data.frame(
  Var1 = c(2/sig_n[sig_sort[i1],nn]),
  Var2 = c(i1))
pointxy.2 <- data.frame(
  Var1 = rep(0,length(i2)),
  Var2 = i2)
g1<-ggplot(data_melt, aes(Var1, Var2)) +
  geom_raster(aes(fill = value), interpolate = TRUE)+
  geom_line(data = pointxy.1,size=0.5)+
  geom_line(data = pointxy.2,size=0.5,color='black')+
