...
##### Main functions
hamt_opt = function(x,sig,f_hat,gd,length_out = 50,bases = 10,likelihood ='Gaussian',df=NULL){
  ...
  if(likelihood=='Gaussian'){
    ...
    for(i in 1:n){
      A[i,]=unlist(sapply(1:m,function(j) rep((1/sig[i])*dt((x[i]-gd[j])/sig[i],df),K)*B[i,]))
    }
  }else if(likelihood=='logistic'){
    for(i in 1:n){
      A[i,]=unlist(sapply(1:m,function(j) rep(dlogis(x[i],gd[j],sig[i]),K)*B[i,]))
    }
  }else if(likelihood=='laplace'){
    for(i in 1:n){
      A[i,]=unlist(sapply(1:m,function(j) rep(dLaplace(x[i],gd[j],sig[i]),K)*B[i,]))
    }
  }else{
    print('Likelihood not supported yet')
  }
  ...
  prob.p <- list(sense="min")
  prob.p$c <- -1*Crossprod(A,f_hat)
  prob.p$A <- rbind(Q_sparse,L_sparse)
  prob.p$bc <- rbind(blc=c(rep(1,n),rep(0,n*m)), 
                     buc=c(rep(1,n),rep(Inf,n*m)))
  prob.p$bx <- rbind(blx=rep(-Inf,m*K), 
                     bux=rep(Inf,m*K))
  
  # Specify the quadratic objective matrix in triplet form.
  prob.p$qobj$i <- c(unlist(sapply(1:(m*K),function(i) i:(m*K))))
  prob.p$qobj$j <- rep(1:(m*K),(m*K):1)
  prob.p$qobj$v <- c(D[lower.tri(D,diag = T)])
  
  # Solve the problem
  out.p <- mosek(prob.p,list(verbose=0))
  if(out.p$sol$itr$solsta=="OPTIMAL"){
    msg = 0
    f = out.p$sol$itr$xx
    ...
    #Solving the relaxed primal
    msg = 1
    prob.p$bc <- rbind(blc=c(rep(0.9,n),rep(0,n*m)), 
                       buc=c(rep(1,n),rep(Inf,n*m)))
    out.p <- mosek(prob.p,list(verbose=0))
    if(out.p$sol$itr$solsta=="OPTIMAL" || out.p$sol$itr$solsta=="UNKNOWN"){
      f = out.p$sol$itr$xx
      f<- matrix(f,ncol=m,byrow=F)
      w_mosek = t(B%*%f)
      for(i in 1:n){
      ...
      msg = 2
      invD = spdinv(D)
      invD = invD+10^{-5}*diag(m*K)
      nu = -1*Crossprod(A,f_hat)
      
      # Specify the non-quadratic part of the problem.
      I_sparse = sparseMatrix(i=1:(m*K),j=1:(m*K),x=1)
      #A1_sparse = sparseMatrix(i=1:(n*m),j=(m*K+n+1):(m*K+n+n*m),x=1)
      A2_sparse = cbind(I_sparse,-t(Q_sparse),t(L_sparse))
      
      prob.d <- list(sense="min")
      prob.d$c <- c(rep(0,m*K),rep(1,n),rep(0,n*m))
      prob.d$A <- A2_sparse
      prob.d$bc <- rbind(blc=c(nu), 
                         buc=c(nu))
      prob.d$bx <- rbind(blx=c(rep(-Inf,m*K+n),rep(0,n*m)), 
                         bux=rep(Inf,m*K+n+n*m))
      
      # Specify the quadratic objective matrix in triplet form.
      prob.d$qobj$i <- c(unlist(sapply(1:(m*K),function(i) i:(m*K))))
      prob.d$qobj$j <- rep(1:(m*K),(m*K):1)
      prob.d$qobj$v <- c(invD[lower.tri(invD,diag = T)])
      
      # Solve the problem
      out.d <- mosek(prob.d,list(verbose=0))
      a1=out.d$sol$itr$slc
      a2 = out.d$sol$itr$suc
      f = a2-a1
      f<- matrix(f,ncol=m,byrow=F)
      w_mosek = t(B%*%f)
      w_mosek[w_mosek<0] = 0
  ...
  return(list('probs'=w_mosek,'msg'=msg))
}

npmleb_opt = function(x,sig,gd,length_out = 50,bases = 10,likelihood ='Gaussian',df=NULL){
  ...
  #--------------------- NPMLE using basis expansion ---------------------
  d = c(t(Q)%*%rep(1/n,n))
  P <- list(sense = "min")
  A0 <- Matrix::Matrix(0, m*K, n)
  P$c <- c(rep(0, n), -rep(1/n,n))
  P$A <- cbind(t(A), A0)
  P$bc <- rbind(rep(0, m*K), d)
  ...
  z <- mosek(P, opts = list(verbose = 1))
  f <- z$sol$itr$suc
  f<- matrix(f,ncol=m,byrow=F)
  aa = t(B%*%f)
  g_npmle=aa%*%diag(1/colSums(aa))
  
  return(list('probs'=g_npmle))
}
...
clfdr.func=function(x,sig,mu0,w,gd,type){
...
clfdr.Lfunc=function(x,sig,mu0,w,gd,type){
  ...
  if(type=='npmle'){
    ...
    # A=mapply(fnullest, x,sig)
    # fnull.dens=t(A)%*%w.null
    fnull.dens = unlist(sapply(1:n,function(i) sum(dLaplace(x[i],gd.null,sig[i])*w.null)))
    ffull.dens = unlist(sapply(1:n,function(i) sum(dLaplace(x[i],gd,sig[i])*w)))
    clfdr=pmin(fnull.dens/ffull.dens,1)
    clfdr=pmax(clfdr,0)
  }
  else if(type=='proposed'){
    ...
    # AA=mapply(fnullest, x,sig)
    # fnull.dens=sapply(1:n,function(i)t(AA[,i])%*%w.null[,i])
    fnull.dens = unlist(sapply(1:n,function(i) sum(dLaplace(x[i],gd.null,sig[i])*w.null[,i])))
    ffull.dens = unlist(sapply(1:n,function(i) sum(dLaplace(x[i],gd,sig[i])*w[,i])))
    clfdr=pmin(fnull.dens/ffull.dens,1)
    clfdr=pmax(clfdr,0)
...
clfdr.func.2=function(x,sig,mu0,mu1,w,gd,type){
...
clfdr.tfunc.2=function(x,sig,mu0,mu1,df,w,gd,type){
  ...
  if(type=='npmle'){
    ...
    # A=mapply(fnullest, x,sig)
    # fnull.dens=t(A)%*%w.null
    fnull.dens = unlist(sapply(1:n,function(i) sum((1/sig[i])*dt((x[i]-gd.null)/sig[i],df)*w.null)))
    ffull.dens = unlist(sapply(1:n,function(i) sum((1/sig[i])*dt((x[i]-gd)/sig[i],df)*w)))
    clfdr=pmin(fnull.dens/ffull.dens,1)
    clfdr=pmax(clfdr,0)
  }
  else if(type=='proposed'){
    ...
    # AA=mapply(fnullest, x,sig)
    # fnull.dens=sapply(1:n,function(i)t(AA[,i])%*%w.null[,i])
    fnull.dens = sapply(1:n,function(i) sum((1/sig[i])*dt((x[i]-gd.null)/sig[i],df)*w.null[,i]))
    ffull.dens = sapply(1:n,function(i) sum((1/sig[i])*dt((x[i]-gd)/sig[i],df)*w[,i]))
    clfdr=pmin(fnull.dens/ffull.dens,1)
    clfdr=pmax(clfdr,0)
...
clfdr.Logisfunc.2=function(x,sig,mu0,mu1,w,gd,type){
  ...
  if(type=='npmle'){
    ...
    # A=mapply(fnullest, x,sig)
    # fnull.dens=t(A)%*%w.null
    fnull.dens = unlist(sapply(1:n,function(i) sum(dlogis(x[i],gd.null,sig[i])*w.null)))
    ffull.dens = unlist(sapply(1:n,function(i) sum(dlogis(x[i],gd,sig[i])*w)))
    clfdr=pmin(fnull.dens/ffull.dens,1)
    clfdr=pmax(clfdr,0)
  }
  else if(type=='proposed'){
    ...
    # AA=mapply(fnullest, x,sig)
    # fnull.dens=sapply(1:n,function(i)t(AA[,i])%*%w.null[,i])
    fnull.dens = sapply(1:n,function(i) sum(dlogis(x[i],gd.null,sig[i])*w.null[,i]))
    ffull.dens = sapply(1:n,function(i) sum(dlogis(x[i],gd,sig[i])*w[,i]))
    clfdr=pmin(fnull.dens/ffull.dens,1)
    clfdr=pmax(clfdr,0)
