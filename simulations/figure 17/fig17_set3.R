...
exp_1<-function(n,mu0,u,q,reps){
  ...
  for(nn in 1:length(u)){
    ...
    result<-foreach(r = 1:reps,.packages=c("ashr","Rmosek","REBayes"))%dopar%{
      ...
      clfdr.or = rep(0,n)
      for(i in 1:n){
        denom = 0.9*dnorm(xbar[i], 0, S[i])+
          0.1*dnorm(xbar[i],3*sig[i],S[i])
        numer = 0.9*dnorm(xbar[i],0,S[i])+
          ((3*sig[i])<=mu0)*0.1*dnorm(xbar[i],3*sig[i],S[i])
      ...
      ################################ ash alpha = 0 #####################
      ash_out = ash(xbar,S,mixcompdist = "normal",mode="estimate",
                    outputlevel=3)
      ...
      ####################################### ash with alpha = 1 ##################
      ash_out = ash(xbar,S,mixcompdist = "normal",alpha=1,outputlevel=3,mode="estimate")
      post_sample_ash=get_post_sample(ash_out,3000)
