...
#Figure 17 Setting 3: t again
load("fig17_set3.RData")
out_exp1_others = out_exp1
load("fig17_set3_adapt.RData")
out_exp1_adapt = out_exp1
load("fig17_set3_camt_ihw.RData")

plotdata1<- as.data.frame(c(colMeans(out_exp1_adapt$fdr.adapt,na.rm=TRUE),
                            colMeans(out_exp1$fdr.camt,na.rm=TRUE),
                            ...
                            colMeans(out_exp1_others$fdr.ash),
                            colMeans(out_exp1_others$fdr.ash.1),
                            colMeans(out_exp1_others$fdr.npmleb)))
names(plotdata1)<-"fdr"
plotdata1$type<-as.factor(rep(c('AdaPTGMM','CAMT','IHW',
                                'GS 1','GS 2','DECONV','HAMT',
                                'ASH','ASH 1','NPMLE B'),each=length(u)))
plotdata1$n<- rep(u,10)
...
plotdata2<- as.data.frame(c(colMeans(out_exp1_adapt$etp.adapt,na.rm=TRUE),
                            colMeans(out_exp1$etp.camt,na.rm=TRUE),
                            ...
                            colMeans(out_exp1_others$etp.ash),
                            colMeans(out_exp1_others$etp.ash.1),
                            colMeans(out_exp1_others$etp.npmleb)))
names(plotdata2)<-"etp"
plotdata2$type<-as.factor(rep(c('AdaPTGMM','CAMT','IHW',
                                'GS 1','GS 2','DECONV','HAMT',
                                'ASH','ASH 1','NPMLE B'),each=length(u)))
plotdata2$n<- rep(u,10)
