...
exp_1<-function(n,mu0,u,q,length_out,bases,reps){
  ...
  for(nn in 1:length(u)){
    ...
    result<-foreach(r = 1:reps,.packages=c("REBayes","MASS","Rmosek","rmutil"))%dopar%{
      ...
      clfdr.or = rep(0,n)
      for(i in 1:n){
        ff<-function(u,x,s,mm){dnorm(x,u,s)*dnorm(u,mm,1)
        } 
        denom = 0.9*dnorm(x[i],0,sig[i])+0.1*integrate(ff,-Inf,Inf,x=x[i],s=sig[i],mm=3)$value
...
save.image(paste(getwd(),'/Setting_1_fig4.RData',sep=''))
