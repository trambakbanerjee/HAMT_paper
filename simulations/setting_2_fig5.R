    ...
    result<-foreach(r = 1:reps,.packages=c("REBayes","MASS","Rmosek","rmutil"))%dopar%{
      ...
      for(i in 1:n){
        denom = 0.9*integrate(ff,-Inf,Inf,x=x[i],s1=sig[i],s2=-sig[i])$value+
          0.1*integrate(ff,-Inf,Inf,x=x[i],s1=sig[i],s2=2*sig[i])$value#integrate(ff,-Inf,Inf,x=x[i],s1=sig[i],s2=u[nn],rel.tol = 1e-15)$value
        numer = 0.9*integrate(ff,-Inf,mu0,x=x[i],s1=sig[i],s2=-sig[i])$value+
          0.1*integrate(ff,-Inf,mu0,x=x[i],s1=sig[i],s2=2*sig[i])$value
...
save.image(paste(getwd(),'/setting_2_fig5.RData',sep=''))
