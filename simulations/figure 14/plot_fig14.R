...
# Figure 14 Setting 3: Laplace likelihood (one-sided)

load("fig14_set3.RData")
out_exp1_others = out_exp1
load("fig14_set3_adapt.RData")
out_exp1_adapt = out_exp1
load("fig14_set3_camt_ihw.RData")
...
plotdata1<- as.data.frame(c(colMeans(out_exp1_adapt$fdr.adapt,na.rm=TRUE),
                            ...
                            colMeans(out_exp1$fdr.camt),
                            colMeans(out_exp1$fdr.ihw)))
names(plotdata1)<-"fdr"
plotdata1$type<-as.factor(rep(c('AdaPTGMM','HAMT',
                                'OR','NPMLE B','CAMT','IHW'),each=length(u)))
plotdata1$n<- rep(u,6)
...
plotdata2<- as.data.frame(c(colMeans(out_exp1_adapt$etp.adapt,na.rm=TRUE),
                            ...
                            colMeans(out_exp1$etp.camt),
                            colMeans(out_exp1$etp.ihw)))
names(plotdata2)<-"etp"
